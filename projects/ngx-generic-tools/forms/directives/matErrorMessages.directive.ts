...
export class MatErrorMessagesDirective implements AfterViewInit {
  ...
  private updateErrors = (state: 'VALID' | 'INVALID') => {
    if (state === 'INVALID') {
      ...
      if (firstError === 'email') {
        this.error = 'El correo introducido no es válido, por favor, revíselo.';
      }
      ...
      if(firstError === 'incorrect'){
        this.error = 'El campo no es correcto';
      }
      if(firstError === 'invalidDni'){
        this.error = 'El identificador introducido no es válido, por favor, revíselo.';
      }
      if(firstError === 'rangoNumero'){
        this.error = 'El número establecido no está dentro del rango.';
      }
    }
  }
}
